...
*Read this in [English](README_old.en.md).*
...
如果遇到了报错 `Could not find module 'nvcuda.dll'` 或者 `RuntimeError: Unknown platform: darwin` (MacOS) ，请[从本地加载模型](README_old.md#从本地加载模型)
...
目前在 MacOS 上只支持[从本地加载模型](README_old.md#从本地加载模型)。将代码中的模型加载改为从本地加载，并使用 mps 后端：
```python
